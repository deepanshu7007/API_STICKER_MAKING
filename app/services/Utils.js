...
exports.get_dates_between_two_dates = async (start_date, end_date) => {
    start_date = moment(start_date).utc();
    end_date = moment(end_date).utc();
    let now = start_date, dates = [];

    // || now.isSame(end_date)
    while (now.isBefore(end_date) || now.isSame(end_date)) {
